...
class Motor_control:
    ...
    def get_angle_l(self):
        angle_l = (self.unitsFC - 1) - (
            (self.l_wheel.read() - self.dcMin_l) * self.unitsFC
        ) / (self.dcMax_l - self.dcMin_l + 1)
        angle_l = max(min((self.unitsFC - 1), angle_l), 0)
        return angle_l

    def get_angle_r(self):
        angle_r = (
            (self.r_wheel.read() - self.dcMin_r)
            * self.unitsFC
